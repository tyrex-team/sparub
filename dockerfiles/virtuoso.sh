...
then
  ...
  /sparub/virtuoso-opensource/binsrc/virtuoso/virtuoso-t +wait && isql-v -U dba -P dba < /dump_nquads_procedure.sql && isql-v -U dba -P dba < /sql-query.sql
...
then
    ...
    /sparub/virtuoso-opensource/binsrc/virtuoso/virtuoso-t +wait && isql-v -U dba -P "$pwd" < /load_data.sql
    kill $(ps aux | grep '[v]irtuoso-t' | awk '{print $2}')
    echo `date +%Y-%m-%dT%H:%M:%S%:z` > .data_loaded
fi

exec /sparub/virtuoso-opensource/binsrc/virtuoso/virtuoso-t +wait #+foreground
